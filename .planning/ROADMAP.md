...
**Plans** : 3 plans (2 vagues)
- [ ] 17-01-PLAN.md — 0017 authoring : wrap RLS `(select …)` toutes policies + matview MRR (wrapper `is_superadmin()` + refresh) + trigger Broadcast + policy `realtime.messages` + Partie B index CONCURRENTLY/script de gate documentés (SCALE-01/02/03/05) — checkpoint A1 (définition MRR)
...
| 17. Fondation DB scalable | v3.0 | 0/3 | Planned | - |
