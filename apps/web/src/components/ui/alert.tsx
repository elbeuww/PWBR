  ...
  "group/alert relative grid w-full gap-0.5 rounded-lg border px-2.5 py-2 text-start text-sm has-data-[slot=alert-action]:relative has-data-[slot=alert-action]:pe-18 has-[>svg]:grid-cols-[auto_1fr] has-[>svg]:gap-x-2 *:[svg]:row-span-2 *:[svg]:translate-y-0.5 *:[svg]:text-current *:[svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        ...
        warning:
          "border-[var(--risk-moderate)]/30 bg-[var(--risk-moderate)]/10 text-[var(--risk-moderate)] *:data-[slot=alert-description]:text-[var(--risk-moderate)] *:[svg]:text-current",
      },
    },
    defaultVariants: {
      variant: "default",
...
function Alert({
  ...
  variant,
  ...props
}: React.ComponentProps<"div"> & VariantProps<typeof alertVariants>) { // i18n-ignore: annotation de type CVA, pas une chaine UI
  return (
    <div
      data-slot="alert"
      role="alert"
